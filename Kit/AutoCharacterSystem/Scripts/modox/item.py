...
class Item(modo.Item):
    ...
    def addUserChannel(self,
        ...
        cmd = '!channel.create "%s" %s item:{%s}' % (name, storageType, self.id)

        if username is not None:
            cmd += ' username:{%s}' % username

        if minVal is not None:
            cmd += ' min:{%s}' % str(minVal)

        if useMin is not None:
            cmd += ' useMin:%d' % useMin

        if maxVal is not None:
            cmd += ' max:{%s}' % str(maxVal)
    ...
    def getUserChannelsNames(self, sort=True):
        ...
        for i in range(chanCount - 1, -1, -1):
            try:
...
class ItemUtils(object):
    ...
    @classmethod
    def getWorldMatrixComposeModifier(self, modoItem, ignoreScale=False):
        ...
        # Taking world scale into consideration is optional.
        if ignoreScale:
            item3 = item1
        else:
            try:
