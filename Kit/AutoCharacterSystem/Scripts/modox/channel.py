...
class ChannelUtils(object):
    ...
    @classmethod
    def isAngleChannel(cls, channel):
        """
        Checks whether given channel is of angle storage type.

        Returns
        -------
        bool
        """
        return channel.storageType == 'angle'
    ...
    @classmethod
    def copyChannels(cls, sourceChannels, targetChannels):
        """ Copies source to target channels.

        Iterates through channels in an order that's set within lists that are arguments.
        Make sure you put channels in correct order into both source and target list.

        Parameters
        ----------
        sourceChannels : [modo.Channel]

        targetChannels : [modo.Channel]
        """
        count = len(sourceChannels)
        count = min(count, len(targetChannels))

        for x in range(count):
            chanSrc = sourceChannels[x]
            chanDst = targetChannels[x]

            sourceIdent = cls.getChannelIdent(chanSrc)
            targetIdent = cls.getChannelIdent(chanDst)
            ...
            lx.out(chanSrc.item.name + ':' + sourceIdent + '  -> ' + chanDst.item.name + ':' + targetIdent)
    ...
    @classmethod
    def restoreToSetup(cls, channels):
        """ Restores given channels to their setup values.

        Note that this erases any animation that channels may have.

        Parameters
        ----------
        channels : [modo.Channel], modo.Channel
            A single or a list of channels to restore to setup.
        """
        if type(channels) not in (list, tuple):
            channels = [channels]

        for chan in channels:
            chanString = cls.getChannelIdent(chan)
            lx.eval('!channel.restoreSetup channel:{%s}' % chanString)
