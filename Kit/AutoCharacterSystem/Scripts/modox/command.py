...
class Command(lxu.command.BasicCommand):
    ...
    def message(self):
        """
        Returns command message object.

        You can use this object to modify command message directly.

        Returns
        -------
        modox.Message
            lx.object.Message wrapped in the more convenient modox version.
        """
    ...
    def cmd_Query(self, index, vaQuery):
        ...
        # We use basestring in python 2 and str in python 3.
        # Since there is no basestring in python 3 NameError exception will happen
        # and str will be used.
        try:
            stringTypeToTest = basestring
        except NameError:
            stringTypeToTest = str

        if isinstance(commander_query_result, stringTypeToTest):
