...
class IK23BarSolver(object):
    ...
    def enabled(self, state):
        ...
        This property does the change on the setup action directly!!!

        Parameters
        ----------
        state : bool
        """
        self.setEnabled(state, time=0.0, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP)
        # if state:
        #     blendVal = 1.0
        # else:
        #     blendVal = 0.0
        # self._modoItem.channel(self._CHAN_BLEND).set(blendVal, time=0.0, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP)

    def setEnabled(self, state, time=None, key=False, action=lx.symbol.s_ACTIONLAYER_EDIT):
        """ Sets IK solving on/off.
        ...
        time : float, None
            None for current action.

        key : bool
            Set the key or not.

        action : str
            One of lx.symbol.s_ACTIONLAYER_XXX
        """
        if state:
            blendVal = 1.0
        else:
            blendVal = 0.0
        self._modoItem.channel(self._CHAN_BLEND).set(blendVal, time=None, key=key, action=action)
    ...
    def scaleOutput(self):
        """
        Gets IK chain output scale values for current time and action.

        Returns
        -------
        [float, float, float]
            3 floats for base, mid and end joint scales.
        """
        base = self._modoItem.channel('scaleAOut').get()
        mid = self._modoItem.channel('scaleBOut').get()
        end = self._modoItem.channel('scaleCOut').get()
        return [base, mid, end]
...
class IK23BarSetup(object):
    ...
    def restoreJointsToSetupPose(self):
        """ Use this to reset IK joints to rest pose, it'll wipe all the animation out.
        """
        joints = self.chain
        for joint in joints:
            chans = LocatorUtils.getItemPositionChannels(joint)
            chans.extend(LocatorUtils.getItemRotationChannels(joint))
            ChannelUtils.restoreToSetup(chans)
