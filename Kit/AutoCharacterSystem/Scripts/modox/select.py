...
class MeshComponentSelection:
    ...
    def __init__(self):
        ...
        self._version = sys.version_info[0]  # Only need to differentiate between python 2 or 3
...
class VertexSelection(MeshComponentSelection):
    ...
    def initModoObject(self, packetPointer):
        ...
        # Mesh Vertex is long/int sensitive.
        # It interprets long index parameter as id,
        # and int as index.
        # So if we want to initialise MeshVertex with id it needs
        # to be long.

        # This is fine for python 2, in python 3 there's no long anymore.
        # For backwards compatibility we need to branch the code and use long or int
        # depending on python version.
        # Python 3 version of TD SDK will handle long/int issue for us, it's enough
        # to just pass an int.
        # There might be nicer way to do this but I'm ok with this for the time being.
        if self._version > 2:
...
class EdgeSelection(MeshComponentSelection):
    ...
    def initModoObject(self, packetPointer):
        ...
        # For explanation on this look into VertexSelection code above.
        if self._version > 2:
            return (mesh, modo.MeshEdge((int(id1), int(id2)), geo))
        else:
        ...
        for edge in edges:
            ...
            for vid in (vid1, vid2):
                if vid not in vertices:
                    vertices[vid] = (edge[0], modo.MeshVertex(int(vid), edge[0].geometry))
        return list(vertices.values())