...
class LocatorUtils(object):
    ...
    @classmethod
    def hasZeroPosition(cls, modoItem):
        """ Tests whether item has zero position transform attached.

        Returns
        -------
        bool
        """
        return cls._getZeroTransformOfType(modoItem, cls.TransformType.POSITION) is not None

    @classmethod
    def hasZeroRotation(cls, modoItem):
        """ Tests whether item has zero rotation transform attached.

        Returns
        -------
        bool
        """
        return cls._getZeroTransformOfType(modoItem, cls.TransformType.ROTATION) is not None

    @classmethod
    def getZeroPosition(self, modoItem, time=None, action=lx.symbol.s_ACTIONLAYER_EDIT):
        """ Gets zero position for an item.
        ...
        ------
        LookupError
            When there's no zero position available on the item.
        """
        if action == lx.symbol.s_ACTIONLAYER_SETUP:
            time = 0.0
        ...
        if zeroPos is not None:
            return modo.Vector3(zeroPos.get(time, action))
        raise LookupError
    ...
    def setZeroPosition(self, modoItem, pos, time=0.0, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP):
        ...
        Note that this function doesn't do anything if there's no zero transform on the item yet.
    ...
    @classmethod
    def getItemRotation(self, modoItem, time=None, action=lx.symbol.s_ACTIONLAYER_EDIT):
    ...
    @classmethod
    def getItemScale(self, modoItem, time=None, action=lx.symbol.s_ACTIONLAYER_EDIT):
    ...
    @classmethod
    def getTransformChannels(cls, modoItem, position=True, rotation=True, scale=True):
        """ Gets item's primary transform channels.

        Channels are returned in order, first position channels, then rotation, then scale.
        ...
        channels = []
        if position:
            pos = cls.getTransformItem(modoItem, c.TransformType.POSITION)
            channels.append(pos.channel(c.TransformChannels.PositionX))
            channels.append(pos.channel(c.TransformChannels.PositionY))
            channels.append(pos.channel(c.TransformChannels.PositionZ))

        if rotation:
            rot = cls.getTransformItem(modoItem, c.TransformType.ROTATION)
            channels.append(rot.channel(c.TransformChannels.RotationX))
            channels.append(rot.channel(c.TransformChannels.RotationY))
            channels.append(rot.channel(c.TransformChannels.RotationZ))

        if scale:
            scl = cls.getTransformItem(modoItem, c.TransformType.SCALE)
            channels.append(scl.channel(c.TransformChannels.ScaleX))
            channels.append(scl.channel(c.TransformChannels.ScaleY))
            channels.append(scl.channel(c.TransformChannels.ScaleZ))
    ...
    @classmethod
    def getPrimaryRotationOrder(cls, modoItem):
        """
        Gets rotation order on a primary rotation transform item.

        Returns
        -------
        str
            One of hints defined by c.RotationOrders.

        Raises
        ------
        LookupError
            When rotation transform item is not found on the modo item.
        """
        xfrmItem = cls.getTransformItem(modoItem, c.TransformType.ROTATION)
        if xfrmItem is None:
            raise LookupError
        return xfrmItem.channel('order').get()

    @classmethod
    def getPrimaryRotationOrder(cls, modoItem):
        """
        Gets rotation order of a primary rotation transform item.

        Parameters
        ----------
        modoItem : modo.Item
            Item to get rotation order from.

        Returns
        -------
        str
            One of c.RotationOrders constants defining an order as a string hint.

        Raises
        ------
        LookupError
            When required rotation transform item cannot be found.
        """
        xfrmItem = cls.getTransformItem(modoItem, c.TransformType.ROTATION)
        if xfrmItem is None:
            raise LookupError
        return xfrmItem.channel('order').get(None, action=lx.symbol.s_ACTIONLAYER_SETUP)

    @classmethod
    def setPrimaryRotationOrder(cls, modoItem, order):
        """
        Sets rotation order on a primary rotation transform item.

        Parameters
        ----------
        modoItem : modo.Item
            Item to set rotation order for.
        ...
        if xfrmItem is None:
            raise LookupError
        xfrmItem.channel('order').set(order, None, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP)

    @classmethod
    def setZeroRotationOrder(cls, modoItem, order):
        """
        Sets rotation order on a zero rotation transform item.
        ...
        LookupError
            When required rotation transform item cannot be found.
        """
        xfrmItem = cls._getZeroTransformOfType(modoItem, cls.TransformType.ROTATION)
        if xfrmItem is None:
            raise LookupError
        xfrmItem.channel('order').set(order, None, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP)
    ...
    def _getZeroTransformFromTheStack(cls, transformsStack, mainXfrmItem):
        ...
        """
        scanIndex = -1
        for x in range(len(transformsStack) - 1):
