...
class DynamicParentSetup(object):
    ...
    @classmethod
    def new(cls, childItem, parentItem=None, name=None, compensation=True, ignoreScale=False):
        ...
        if parentItem is not None:
            ...
            newSetup.addParent(parentItem, compensation=compensation, ignoreScale=ignoreScale)
    ...
    def addParent(self,
        ...
        matrixCompose = self._addMatrixCompose(parentItem, ignoreScale=ignoreScale)
    ...
    def setParent(self,
        ...
        if self._pyVersion < 3:
            typesToTest = [int, long]

        if type(itemOrIndex) in typesToTest:
            index = itemOrIndex
    ...
    def matrixComposerModifiers(self):
        ...
        for x in range(inputChan.revCount):
    ...
    def _addMatrixCompose(self, parentItem, ignoreScale=False):
        ...
        modo.Item
            ...
            automatically.
        """
        matrixCompose = ItemUtils.getWorldMatrixComposeModifier(parentItem, ignoreScale=ignoreScale)
...
class DynamicParentModifier(object):
    ...
    @property
    def offset(self):
        """ Gets position and rotation offset from dynamic parent modifier as 2 vectors.

        Scale is not returned as scale offsets do not work reliably anyway
        ...
        """
        px = self._item.channel(self.DYNA_PARENT_CHANNELS[1]).get()
        py = self._item.channel(self.DYNA_PARENT_CHANNELS[2]).get()
        pz = self._item.channel(self.DYNA_PARENT_CHANNELS[3]).get()

        rx = self._item.channel(self.DYNA_PARENT_CHANNELS[4]).get()
        ry = self._item.channel(self.DYNA_PARENT_CHANNELS[5]).get()
        rz = self._item.channel(self.DYNA_PARENT_CHANNELS[6]).get()

        return modo.Vector3(px, py, pz), modo.Vector3(rx, ry, rz)
