...
class DeformFolder(object):
    ...
    def enabled(self):
        """
        Gets deform folder enable state.

        Returns
        -------
        bool
        """
        return bool(lx.eval('deformer.enable {%s} ?' % self._item.id))

    @enabled.setter
    def enabled(self, state):
        """
        Sets deform folder enable state.

        Parameters
        ----------
        state : bool
        """
        lx.eval('!deformer.enable {%s} %d' % (self._item.id, int(state)))
...
class Deformer(object):
    ...
    @effector.setter
    def effector(self, effectorModoItem):
        """
        Sets new effector item for deformer.

        Only one effector is allowed.

        Parameters
        ----------
        effectorModoItem : modo.Item, None
            Pass None to clear effector assignment.
        """

        item.ItemUtils.clearReverseGraphConnections(self._item, GRAPH_DEFORMERS)

        if effectorModoItem is not None:
            influenceGraph = self._item.itemGraph(GRAPH_DEFORMERS)
            effectorGraph = effectorModoItem.itemGraph(GRAPH_DEFORMERS)
            effectorGraph >> influenceGraph
...
class MorphInfluence(Deformer):

    @property
    def mapName(self):
        """ Gets morph map name that this influence is using.

        Returns
        -------
        str, None
        """
        return self._item.channel('mapName').get(0.0, lx.symbol.s_ACTIONLAYER_EDIT)
    ...
    def mapName(self, name):
        ...
        name : str
        """
        self._item.channel('mapName').set(name, time=0.0, key=False, action=lx.symbol.s_ACTIONLAYER_SETUP)