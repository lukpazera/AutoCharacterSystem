...
class Assembly(object):
    ...
    @classmethod
    def isInputChannel(cls, channel):
        """ Tests whether given channel is assembly's input channel.
        ...
        """
        item = channel.item
        graph = lx.object.ChannelGraph(item.scene.GraphLookup('assemblyChans'))
        if graph.RevCount(item, channel.index) > 0:
            return True
        return False
