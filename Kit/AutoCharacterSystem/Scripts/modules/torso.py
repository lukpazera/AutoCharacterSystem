...
KEY_ROOT_TIP_CONTROLLER_GUIDE = 'roottipedgd'
...
class HorizontalVariant(rs.base_ModuleVariant):
    ...
    def onApply(self):
        ...
        rootTipGuide = self.module.getKeyItem(KEY_ROOT_TIP_CONTROLLER_GUIDE)

        rootGuidePosVec = modo.Vector3()
        pos = modox.LocatorUtils.getItemPosition(rootTipGuide.modoItem, 0.0, lx.symbol.s_ACTIONLAYER_EDIT)
        rootGuidePosVec.z = pos.z * 1.2
        modox.LocatorUtils.setItemPosition(rootGuide.modoItem, rootGuidePosVec, 0.0, key=False, action=lx.symbol.s_ACTIONLAYER_EDIT)
...
class VertebraPiece(rs.base_SerialPiece):
    ...
    descModuleHierarchy = {k.ROOT_CONTROLLER_GUIDE: k.CONTROLLER_GUIDE,
...
class VertebraeSetup(rs.base_SerialPiecesSetup):
    ...
    descGuideChainStart = k.ROOT_CONTROLLER_GUIDE
...
class TorsoModule(rs.base_FeaturedModule):

    descIdentifier = 'std.torso'
    descUsername = 'Torso'
